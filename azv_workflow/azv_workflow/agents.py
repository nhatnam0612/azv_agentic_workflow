...
checker_agent = Agent(
    ...
    instructions=(
        ...
        ""
        "MANDATORY DATE CHECK: Before finalizing any assessment, explicitly compare "
        "treatment_period against the date field of EVERY entry in `lines`, and also compare "
        "both against invoice_date and the contract's Versicherungsbeginn. These dates must be "
        "mutually consistent. If treatment_period contradicts the line-level dates (e.g. "
        "treatment_period says a different year than the individual line dates), or if any "
        "date is implausible given the contract, this is a certain sign of a misread field — "
        "you MUST call `request_reextraction` and ask Agent 1 to specifically re-check "
        "treatment_period (and any other affected date field) BEFORE you proceed with or "
        "finalize your assessment. This check and the resulting tool call are REQUIRED, not "
        "optional — do not reason around a suspected date error, do not simply note it in your "
        "reasons, and do not classify the case while a suspected date inconsistency is "
        "unresolved. Request the re-extraction first, then assess using the (possibly "
        "corrected) result. "
        ""
        "For other kinds of suspected extraction errors (non-date fields), you may also call "
        "`request_reextraction` with a precise field hint and reason, following the same "
        "principle: only for suspected extraction errors, not for facts genuinely absent from "
        "the invoice, and not because you believe the contract itself is missing — the contract "
        "was already given to you directly in this prompt, not to Agent 1. "
        "You may call the tool at most twice in total. If evidence is still insufficient "
        "afterwards, return UNKLAR. This is decision support, not a binding decision. "
...
def _inject_extraction_error(extraction: InvoiceExtraction) -> InvoiceExtraction:
    """Nur für Demo-/Testzwecke: verfälscht absichtlich treatment_period, sodass ein klarer,
    decision-relevanter innerer Widerspruch zu den unveränderten Zeilen-Daten (lines[].date,
    weiterhin '19' = 2019) entsteht und request_reextraction zuverlässiger ausgelöst wird."""
    corrupted = copy.deepcopy(extraction)
    original_period = corrupted.treatment_period
    corrupted.treatment_period = "05.10.2020 bis 10.10.2020"
    logger.warning(
        "[TESTMODUS] Extraktion künstlich verfälscht: treatment_period '%s' -> '%s' "
        "(widerspricht lines[].date von 2019)",
        original_period, corrupted.treatment_period,
    )
    return corrupted
...
def run_workflow(
    ...
    logger.info("=== SCHRITT 2: AGENT 2 – AZV-Bewertung ===")
    logger.info(
        "[DEBUG] contract_text Länge: %d Zeichen | Vorschau: %r",
        len(contract_text), contract_text[:150],
    )
    deps = CheckerDeps(
...
Remember: if you notice any date-related inconsistency, request a re-extraction before assessing.
"""
